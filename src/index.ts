...
const urls = {
  ...
  module_aws_s3:
    "https://registry.terraform.io/v1/modules/terraform-aws-modules/s3-bucket/aws",
  module_aws_iam:
    "https://registry.terraform.io/v1/modules/terraform-aws-modules/iam/aws",
  tfaction:
    "https://api.github.com/repos/suzuki-shunsuke/tfaction/releases/latest",
  aqua: "https://api.github.com/repos/aquaproj/aqua/releases/latest",
  aqua_installer:
    "https://api.github.com/repos/aquaproj/aqua-installer/releases/latest",
  aqua_registry:
  ...
  github_comment:
    "https://api.github.com/repos/suzuki-shunsuke/github-comment/releases/latest",
 ...
 */
async function renderTemplate(templatePath: string, variables: object) {
  try {
    ...
    for (const entry of entries) {
      ...
      if (entryStats.isDirectory()) {
        ...
        fs.writeFile(entryPath, nunjucks.render(entryPath, variables));
      }
    }
  } catch (error) {
    console.error(error);
...
async function getVersions() {
  async function fetchVersions(url: string) {
    ...
    try {
      ...
      console.warn(
        `⚠️ ${url} からバージョン情報を取得できませんでした。デフォルトバージョンを設定します。`
      );
      // エラーが発生した場合、nullを返却
      return null;
    }
  ...
  const [
    ...
    tfactionVersion,
    aquaVersion,
    aquaInstallerVersion,
    aquaRegistryVersion,
    tflintVersion,
    tflintRulesetAwsVersion,
    trivyVersion,
    reviewdogVersion,
    terraformDocsVersion,
    githubCommentVersion,
    ghcpVersion,
  ] = await Promise.all([
  ...
  const versions = {
    ...
    terraform_version_minor:
      terraformVersion?.split(".").slice(0, 2).join(".") ||
    ...
    aqua_installer_version:
      aquaInstallerVersion || defaultVersions.aqua_installer,
    ...
    tflint_ruleset_version_aws:
      tflintRulesetAwsVersion || defaultVersions.tflint_ruleset_aws,
    trivy_version: trivyVersion || defaultVersions.trivy,
    reviewdog_version: reviewdogVersion || defaultVersions.reviewdog,
    terraform_docs_version:
      terraformDocsVersion || defaultVersions.terraform_docs,
    github_comment_version:
      githubCommentVersion || defaultVersions.github_comment,
...
program
  ...
  .option(
    "-t, --target <path>",
    "The target path to generate the code. default: current directory"
  )
